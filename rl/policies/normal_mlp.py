...
class CvaMLPPolicy(Policy, nn.Module):
    """CVA network based on a multi-layer perceptron (MLP), with a
    `Normal` distribution output, with trainable standard deviation. This
    policy network can be used on tasks with continuous action spaces (eg.
    `HalfCheetahDir`).
    """

    def __init__(self, input_size, output_size, device, hidden_sizes=(), num_context_params=10, embedding_size=10,
                 nonlinearity=F.relu, init_std=1.0, min_std=1e-6):
        super(CvaMLPPolicy, self).__init__(input_size, output_size)
        self.input_size = input_size
        self.output_size = output_size
        self.device = device
        ...
        self.embedding = nn.Embedding(embedding_size, num_context_params)
    ...
    def forward(self, input, params=None):
        ...
        # split the input and pass task id through embedding layer
        # concatenate input with context parameters
        latent = input[:, self.input_size:]
        input = input[:, :self.input_size]
        latent = self.embedding(latent.flatten().long()).float()
        input = torch.cat((input.reshape(-1, self.input_size), latent.reshape(-1, self.num_context_params)), dim=1).float()
